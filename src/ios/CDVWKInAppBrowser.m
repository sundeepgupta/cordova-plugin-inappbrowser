...
- (void)open:(CDVInvokedUrlCommand*)command
{
    ...
    if (url != nil) {
        ...
        if ([target isEqualToString:kInAppBrowserTargetSelf]) {
            ...
            if ([absoluteUrl.absoluteString rangeOfString:@"data:text/html;base64,"].location == NSNotFound) { // Normal flow
                [self openInInAppBrowser:absoluteUrl withOptions:options];
            } else {
                [self openInInAppBrowserOneCart:absoluteUrl withOptions:options];
            }
        }
...
- (void)openInInAppBrowserOneCart:(NSURL*)url withOptions:(NSString*)options
{
    CDVInAppBrowserOptions* browserOptions = [CDVInAppBrowserOptions parseOptions:options];
    if (_oneCartViewController == nil) { // create and hide it
        NSLog(@"foo _oneCartViewController is nil, so create and hide it");

        _oneCartViewController = [[CDVWKInAppBrowserViewController alloc] initWithBrowserOptions: browserOptions andSettings:self.commandDelegate.settings processPool:_processPool]; // fixme: make sure we're caching everythign
        _oneCartViewController.navigationDelegate = self;

        if ([self.viewController conformsToProtocol:@protocol(CDVScreenOrientationDelegate)]) {
            _oneCartViewController.orientationDelegate = (UIViewController <CDVScreenOrientationDelegate>*)self.viewController;
        }

        [_oneCartViewController showLocationBar:browserOptions.location];
        [_oneCartViewController showToolBar:browserOptions.toolbar :browserOptions.toolbarposition];
        if (browserOptions.closebuttoncaption != nil || browserOptions.closebuttoncolor != nil) {
            int closeButtonIndex = browserOptions.lefttoright ? (browserOptions.hidenavigationbuttons ? 1 : 4) : 0;
            [_oneCartViewController setCloseButtonTitle:browserOptions.closebuttoncaption :browserOptions.closebuttoncolor :closeButtonIndex];
        }
        ...
        _oneCartViewController.modalPresentationStyle = presentationStyle;
        ...
        _oneCartViewController.modalTransitionStyle = transitionStyle;

        //prevent webView from bouncing
        if (browserOptions.disallowoverscroll) {
            if ([_oneCartViewController.webView respondsToSelector:@selector(scrollView)]) {
                ((UIScrollView*)[_oneCartViewController.webView scrollView]).bounces = NO;
            } else {
                for (id subview in _oneCartViewController.webView.subviews) {
...
- (void)setupOneCartViewController:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate displayCustomToolbar:(BOOL)displayCustomToolbar toolbarTitle:(NSString *)toolbarTitle toolbarForegroundColor:(UIColor *)toolbarForegroundColor toolbarBackgroundColor:(UIColor *)toolbarBackgroundColor
{
    _oneCartViewController.navigationController.navigationBarHidden = !displayCustomToolbar;

    if (_oneCartViewController == nil) {
    ...
    __block CDVInAppBrowserNavigationController* nav = [[CDVInAppBrowserNavigationController alloc]
                                                        initWithRootViewController:_oneCartViewController];
    nav.orientationDelegate = _oneCartViewController;
    nav.navigationBarHidden = YES;
    nav.navigationBar.barTintColor = toolbarBackgroundColor ?: UIColor.blackColor;
    nav.modalPresentationStyle = _oneCartViewController.modalPresentationStyle;
    nav.presentationController.delegate = _oneCartViewController;
    ...
    if (displayCustomToolbar) {
        ...
        [button addTarget:_oneCartViewController action:@selector(close) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *barButtonItem = [[UIBarButtonItem alloc] initWithCustomView:button];
        _oneCartViewController.navigationItem.leftBarButtonItem = barButtonItem;
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (strongSelf->_oneCartViewController != nil) {
            if (!strongSelf->_oneCartWindow) {
                CGRect frame = [[UIScreen mainScreen] bounds];
                frame.origin.x = frame.size.width;
                strongSelf->_oneCartWindow = [[UIWindow alloc] initWithFrame:frame];
            }
            UIViewController *tmpController = [[UIViewController alloc] init];
            [strongSelf->_oneCartWindow setRootViewController:tmpController];
            [strongSelf->_oneCartWindow setWindowLevel:UIWindowLevelNormal];

            [self->_oneCartWindow makeKeyAndVisible];
